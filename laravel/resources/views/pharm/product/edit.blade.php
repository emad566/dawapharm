...
@section('headMeta')
    <title>تعديل المنتج: {{$product->p_name}}</title>
@endsection
...
<div class="container">
    <div class="panel panel-default">
        <div class="panel-heading">
            تعديل المنتج: {{ $product->p_name }}
            <a href="{{ url('pharm/product?p=product') }}" class="btn btn-primary action-btn pull-left"><i class="fas fa-coffee"></i> كل المنتجات </a>  
            <a href="{{ url('pharm/product/create?p=product') }}" class="btn btn-primary action-btn pull-left"><i class="fas fa-calendar-plus"></i> أضف جديد</a>
            <a href="{{ url('pharm/product/'.$product->id.'?p=product') }}" class="btn btn-primary action-btn pull-left"><i class="fas fa-eye"></i> عرض</a>
        ...
        <div class="panel-body panelBodyForm">
            ...
            <form id='CreateUser' class='CreateUser form-horizontal panelForm' method='POST' action="{{ route('product.update', [$product->id]) }}">
                {{ csrf_field() }}
                ...
                <div class="form-group{{ $errors->has('p_name') ? ' has-error' : '' }}">
                    ...
                    <input value="@if(old('p_name'))@else{{$product->p_name}}@endif" type="text" name="p_name"  required class="form-control name" id="p_name" placeholder="اسم المنتج">
                ...
                <div class="form-group{{ $errors->has('p_active_ingredient') ? ' has-error' : '' }}">
                    ...
                    <input value="@if(old('p_active_ingredient'))@else{{$product->p_active_ingredient}}@endif" type="text"  name="p_active_ingredient"  class="form-control name" id="p_active_ingredient" placeholder="المادة الفعالة ">
                ...
                <div class="form-group{{ $errors->has('p_price') ? ' has-error' : '' }}">
                    ...
                    <input value="@if(old('p_price'))@else{{$product->p_price}}@endif" type="number" min="0" max='99999999999' step="0.01" required name="p_price" class="form-control name" id="p_price" placeholder="سعر المنتج *">
                ...
                <div class="form-group{{ $errors->has('sort_factor') ? ' has-error' : '' }}">
                    ...
                    <input value="@if(old('sort_factor'))@else{{$product->sort_factor}}@endif" type="number" min="0" max='99999999999' step="0.01" required name="sort_factor" class="form-control name" id="sort_factor" placeholder="معامل الترتيب">
                            ...
                            @foreach($offers as $offer)
                                <li>
                                    <label for="offer{{ $offer->id }}">
                                        ...
                                        <input type="checkbox" value="{{ $offer->id }}" {{$checked}} name="offers[]" id="offer{{ $offer->id }}">
